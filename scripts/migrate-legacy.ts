...
function migrationSql(
  ...
  for (const record of records) {
    ...
    const inferredBmw =
      record.category === "Big class 3 test" &&
      (/\bBMW\b|\b3 Series\b|\b4\s*series\b/i.test(
        `${record.title} ${record.model}`,
      ) ||
        /^(?:WBA|5UX)/i.test(record.vin ?? ""));
    const normalizedTitle =
      inferredBmw && !/\bBMW\b/i.test(record.title)
        ? record.title.replace(/^(\d{4})\s+/, "$1 BMW ")
        : record.title;
    const make = inferredBmw ? "BMW" : record.category;
    const exteriorColor = /^SLIVE$/i.test(record.exteriorColor ?? "")
      ? "SILVER"
      : record.exteriorColor;
    const title = quote(normalizedTitle);
    ...
    if (process.env.VERIFY_REMOTE === "1") {
      ...
      try {
        ...
        try {
          ...
          const uniqueRemoteImages = manifest.filter(
            (item) => item.ok === true && item.duplicateOf == null,
          );
          const sampleIndexes = [
            0,
            Math.floor(uniqueRemoteImages.length / 2),
            uniqueRemoteImages.length - 1,
          ];
          const sample = [
            ...new Set(
              sampleIndexes
                .map((index) => uniqueRemoteImages[index])
                .filter(Boolean)
          ...
          result.remoteR2SampleCount = sample.length;
        } catch (error) {
